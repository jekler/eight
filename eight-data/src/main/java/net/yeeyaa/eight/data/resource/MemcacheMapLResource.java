...
public class MemcacheMapLResource<K, V> implements IListableResource<K, V>, IExtendable<Object> {
	...
	protected final IProcessor<K[], Object> count = new IProcessor<K[], Object>(){
		...
		public Object process(final K[] paras) {
			...
			} else map = (Map<K, V>)mc.get(key);
			return new Long(map.size());
		}
	};
	
	public MemcacheMapLResource() {
		log = LoggerFactory.getLogger(MemcacheMapLResource.class);
	}

	public MemcacheMapLResource(Logger log) {
		this.log = log == null ? LoggerFactory.getLogger(MemcacheMapLResource.class) : log;
	}
	...
	public V find(K ... paras) {
		if(paras != null && paras.length > 0 && paras[0] != null)  {
			...
			if(map != null) return map.get(paras[0]);
		}
		return null;
	}
