...
public class BundleCenter implements IBundleService, IProcessor<Object, Object>, IListableResource<Object, String>, IExtendable<Object>, BundleContext {
	protected static final MapperSet<Object> methods = new MapperSet<Object>(new Object[]{ResourceMethod.count});
	protected final Logger logger;
	protected final BundleContext bundleContext;
	protected final Map<String, List<IBundleProxy>> serviceMap = new ConcurrentHashMap<String, List<IBundleProxy>>();
	...
	protected volatile Map<String, Set<String>> permit = new ConcurrentHashMap<String, Set<String>>(); //empty for permitting all bean; empty set for permitting all method; a non-exist key for to deny all(strange)
	...
	protected Integer trace = 0;//0: close cache; positive: cache size for exception response; negative: cache size for all response
	protected Boolean clone;//null: cache origin request and response; true: cache copy for request and resposne; false: cache copy for request and origin for response
	protected Long wait = 5000L;
	protected ExecutorService executor; 
	protected Boolean thread;//null: don't create thread, use executor ; true: create thread if executor not exist; false: don't use thread;
	protected BundleService<Object> hook;
	protected String resource;
	protected int mode; //0 cp -> local 1 fs -> local 2 cp 3 fs 4 none other: local
	...
	protected enum MethodType{execute, find, store, delete, empty, keys, all, process, perform, operate, realObject, extend, methods, present;}
	...
	protected void bindExecutor(ExecutorService executor) {
		this.executor = executor;
	}

	protected void unbindExecutor() {
		this.executor = null;
	}
	...
	protected Map<String, List<IBundleProxy>> getServiceMap() {
		return serviceMap;
	}
    ...
    protected void bindProxy(IBundleProxy service){
		if(service != null && service.getName() != null) synchronized(serviceMap) {
			List list = serviceMap.get(service.getName());
			if(list == null) {
				list = new CopyOnWriteArrayList<IBundleProxy>();
    ...
    protected void unbindProxy(IBundleProxy service){
		if(service != null && service.getName() != null) synchronized(serviceMap) {
			unavailable.remove(service.getName());
			List<IBundleProxy> list = serviceMap.get(service.getName());
			if(list != null) {
				list.remove(service);
  ...
  //trace: null for tmp status; positive for return tmp (cache on tmp null) index element; -1 for cache -> tmp; -2 for tmp -> null; -3 for cache list; -4 for tmp list; other negative for reinvoke -debug-10 tmp (cache on tmp null) index element and return result; 
	protected Object trace(Integer trace, Boolean mode) {//mode: true for return key; false for return value; null for return both
    ...
    //mode: null for restart; true for start; false for stop. status: null for hungup restart and restore to former status, true for hungup restart and always reboot, false for no hungup restart.
    protected Boolean control (final Boolean mode, final Boolean status, Boolean flag) {//flag: null for center config; true for new thread; false for current thread
    	...
    	if (flag == null) {
    		...
    		tasks.put(task, t);
    		t.start();
    	} else if (flag) tasks.put(task, executor.submit(task));
    	else {
    ...
    //state: null for return state; true for valid; false for invalid; 
	protected Boolean state(Boolean state, Boolean kill) {//kill: null for set state; true for kill all pre tasks and set state; false for kill counterpart tasks
		if (kill == null) if (state == null) return this.state; //kill -> null + state -> null = do nothing and return state;
		else {
			this.state = state;
			return startSignal.getCount() > 0; //kill-> null + state -> null = set state and return signal;
		} else { //return task status;
			Boolean ret = tasks.size() > 0;
			if (kill && state != null) if (state) synchronized(tasks) {//kill -> true + state -> true, false = set signal;
				if (startSignal.getCount() == 0) startSignal = new CountDownLatch(1);
				...
				while (itr.hasNext()) try {
					...
					if (kill || (state == null ?  entry.getKey().mode == null : state.equals(entry.getKey().mode))) { //kill -> false + state -> null, true, false = three type kill; kill -> true + state -> null = all kill;
						itr.remove();																	
    ...
    protected void validate(){
    	if (beanHolder == null) synchronized(tasks) {
    ...
    protected void updated(Dictionary<String, Object> d) {
    	if(d != null) synchronized (unavailable) {
					...
					try {
						switch (method) {
							...
							case store: if(service instanceof IOutputResource && paras instanceof Object[]) {
								...
								if(p.length > 1 && p[1] instanceof Object[]) return (ret = ((IOutputResource)service).store(p[0], (Object[])p[1]));
							}
							break;
							...
							break;
							case execute: if(service instanceof ITransaction && paras instanceof IProcessor) return (ret = ((ITransaction)service).execute((IProcessor)paras));
							break;				
	...
	protected Object getService(String name) {
		IBundleProxy service = null;
		List<IBundleProxy> list = serviceMap.get(name);
		if(list != null) try{
