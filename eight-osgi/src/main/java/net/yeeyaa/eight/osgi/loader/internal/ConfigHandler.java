package net.yeeyaa.eight.osgi.loader.internal;
...
public class ConfigHandler{
	...
	public static void main(String[] args) {
		if (args != null && args.length > 0) try {
			...
			byte[] bin = TypeConvertor.urlToBytes(TypeConvertor.strToUrl(args[0]), 8192, -1L);
			if (args.length > 2) TypeConvertor.bytesToFile(TypeConvertor.decode(new String(bin, "UTF-8")), args[1]);
			else {
				if (bin[0] != 80) bin = TypeConvertor.compress(bin, -8192, null);
				String content = TypeConvertor.encode(bin);
