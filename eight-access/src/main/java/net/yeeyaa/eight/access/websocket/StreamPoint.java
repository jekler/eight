		...
		try {
			...
			if (length > 5) {
				...
				if (filter == null || filter.operate(session, id, flag)) if ((flag & 0x80) == 0x80) {
					...
					if ((flag & 0x40) == 0x40) {
	/*						Object ret = stream.process(new Para(new ByteArrayInputStream(data), map));
						if (ret != null && !listener) if (ret instanceof byte[]) responseBody.write((byte[]) ret);
						else if (ret instanceof Wrapper) ((Wrapper)ret).processor.process(responseBody);
						else {
							InputStream in = ret instanceof IStorage ? ((IStorage) ret).getInputStream() : ret instanceof InputStream ? (InputStream) ret : null;
