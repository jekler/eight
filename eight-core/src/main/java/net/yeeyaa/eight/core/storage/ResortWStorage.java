...
public class ResortWStorage<T, R extends IExtendable<Object>> extends Storage<Object> {
	...
	protected T[] keys;
	protected Integer mode = 0;//0缓存不检测重试 1缓存检测重试 2缓存不检测不重试 3缓存检测重试 其它 不缓存
	...
	}//UniversalProxy + MatchProcessor + callback + ResortWStorage == new InputResource + ResortWStorage factory
