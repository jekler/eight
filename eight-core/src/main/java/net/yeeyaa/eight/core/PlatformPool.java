...
public class PlatformPool implements IProcessor<Object, Object>, ITriProcessor<Object, Object, Object, Object>, ScheduledExecutorService, RejectedExecutionHandler{
	...
	protected class LocalRunnable implements Runnable, IBiProcessor<Object, Object, Object> {
        ...
        protected Boolean mode;//true: thread local; false: exception; null: both
        protected UUID id;
		...
		public LocalRunnable(Runnable task, Boolean mode) {
            ...
            if (!Boolean.FALSE.equals(mode)) try {
                ...
                log.error("PlatformPool: error when gets threadlocals.", e);
            }
        }
	...
	protected Object comparator;//prior queue camparator
	protected Integer size;
	protected Integer max;
	protected Integer queue = 0x80000001; //0 : sync; <0 link; 0> & comparator == null: array; >0 & comparator != null: prior
	protected Boolean mode;
	protected Boolean timeout = false; //core thread can be timeout
	protected Long alive = 60000000000L;
	protected Boolean now = false; //shutdown now?
	protected Boolean chain = true; //chain shutdown the handler
	protected Boolean type; //false: sche; other: exec
	protected volatile Integer cache;// null: no cache; first bit: 1 -> all; 0 -> only processor; second bit: 1 -> begin; 0 -> only run; third bit: 1 -> don't remove; 0 -> remove
	...
	public static class Cleaner<T> implements IProcessor<T, T> {
		protected final Logger log;
		protected Boolean mode; //null: callback and clean, true: only clean, false: only callback
		protected Boolean inherit; //null: both; true: inheritable; false; not inheritable;
	        	...
	        	if (!Boolean.FALSE.equals(inherit)) {
		            ...
		            if(threadLocalTable != null) {
			            ...
			            if(table != null) for (int j = 0; j < table.length; j++) if (table[j] instanceof WeakReference) try {
	                        ...
	                        log.error("PlatformPool: execute callback fail.", e);
	                    } 
		            }
	        	}
