...
public class VoidProcessor<T, R> implements IProcessor<T, R> {//mask PlatformBean, Proxy -->PlatformReference
	...
	@Override
	public R process(T instance) {//thread wait before platformBean begins. this helps some init methods (no dependence) going on after platformBean begin.
		if(processor == null) if(Boolean.TRUE.equals(stub)) return (R)instance; //stub mocks a processor interface and do nothing. UniversalProxy + sutb = dumb mocks 
		else return null;
	...
	public void initialize(){//do some init works in spring config
