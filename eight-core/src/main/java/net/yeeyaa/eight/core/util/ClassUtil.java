...
public class ClassUtil<T> {
	...
	public List<String> getClassList(String path){
    	...
    	if (path != null) try{
    		if (path.trim().toLowerCase().endsWith(".jar")){ //如果是jar文件,遍历其中类名
    		...
    		else if (path.trim().toLowerCase().endsWith(".zip")){//如果是zip文件,同上	            			
    			File file = new File(path);
    			if (file.exists()){
    		...
    		else if (path.trim().toLowerCase().endsWith(".class")){//如果是单独的class,加入类名
    			File file = new File(path);
    			if(file.exists()){
    				clslist.add(file.getName().substring(0, file.getName().length()-6));
    			}
    		}
    		else {//如果是目录遍历其中加入类名,以这个目录为基路径
	...
	public ClassLoader getClassLoader(List<String> paths){
		...
		for(String path : paths)try{
	    	if (path != null){
	    		if (path.trim().toLowerCase().endsWith(".jar")){ //如果是jar文件,加入loader路径
	    			File file = new File(path);	            			
	    			if(file.exists()){
	    		...
	    		else if (path.trim().toLowerCase().endsWith(".class")){//如果是单独的class,加入路径
	    			File file = new File(path);
	    			if(file.exists()){
	    				pathlist.add(file.getParentFile().toURI().toURL());
	    			}
	    		}
	    		else {//如果是目录,以这个目录为基路径
			        File dir = new File(path);        		        
			        if (dir.exists()) {
