			...
			for (Entry<IProcessor<Object, Object>, Object> entry : generator) {
				...
				if (ret != null) {
					...
					if (value instanceof Integer) {
						...
						if (length < 0) {
							length = -length;
							for (int i = bytes.length - 1, min = (length > bytes.length ? 0 : bytes.length - length), j = 0; i >= min;  i--, j++) if (index + j > 15) break; //last bytes desc order
							else bs[index + j] = bytes[i];
						} else if (length > 0) for (int i = length > bytes.length ? 0 : bytes.length - length, j = 0; i < bytes.length;  i++, j++) if (index + j > 15) break; //last bytes asc order
						else bs[index + j] = bytes[i];
