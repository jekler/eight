...
public class ResourceRemoveProcessor<K, V> implements IProcessor<Object, Collection<K[]>> {
	...
	protected Boolean isArray = false;//whether input parameters is already an array or a single object
	protected Boolean reserve; //true reserve in only (remove not in): false reserve not in only (remove in): null do nothing but return not in collection
