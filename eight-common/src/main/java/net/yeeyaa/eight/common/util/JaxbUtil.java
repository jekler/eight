...
public class JaxbUtil implements IProcessor<Object, JAXBContext> {
    ...
    protected Boolean lazyInit; //true: don't create context; null: default, safemode, don't invoke propertyProcessor in init; false: init context and invoke propertyProcessor
	...
	public synchronized JAXBContext process(Object instance) {
		try {
	    	...
	    	jc = null;
	    	if (jcf != null) try {
	    		jc = (JAXBContext) jcf.invoke(null, contextPath, classLoader, Boolean.TRUE.equals(instance) /*init*/ || propertyProcessor == null ? properties : propertyProcessor.process(properties));
	    	} catch (Exception e) {
	    		log.error("JaxbUtil: new context can not be loaded.", e);
	    	}
	    	if (jc == null) jc = JAXBContext.newInstance(contextPath, classLoader, Boolean.TRUE.equals(instance) /*init*/ || propertyProcessor == null ? properties : propertyProcessor.process(properties));
