...
public class UnserializeProcessor implements IProcessor<Object, Object> {
	protected Boolean overlap = false; //false: not override, true: override factory and initValue, null: override factory only;
	protected Integer mode = 0; //0: return initial processor result, 1: return initial processor result, in case of null return factory result directly, 2: return factory result without initial (IMetaBean or Processor).
	protected Boolean copy = false;
	protected Boolean retry = false; //enhance process twice to conform convert haven been done
	...
	protected Object unserialize(Object instance, Map<String, Object> map){
		if(instance instanceof BeanMeta) {
			...
			if(meta.getRef()) {
				...
				if (Boolean.TRUE.equals(overlap)) initValue = this.initValue; //if initValue contains BeanMeta and ret is processor, the processor should do with BeanMeta properly by itself
